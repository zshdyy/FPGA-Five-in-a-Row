...
architecture Behavioral of piece_render is
    ...
    constant PIECE_RADIUS : integer := 20;
...
begin
    ...
    begin
        ...
        -- 修改边界检查为条件执行块
        if x_pos >= BOARD_OFFSET_X and x_pos < BOARD_OFFSET_X + BOARD_WIDTH and
           y_pos >= BOARD_OFFSET_Y and y_pos < BOARD_OFFSET_Y + BOARD_HEIGHT then
            
            -- 精确计算格子位置
            cell_x := (x_pos - BOARD_OFFSET_X) / (CELL_SIZE+LINE_WIDTH) ;
            cell_y := (y_pos - BOARD_OFFSET_Y) / (CELL_SIZE+LINE_WIDTH);
            
            
            -- 获取格子状态（2bit）
            cell_state := board_state((cell_y*BSIZE + cell_x)*2 + 1 downto (cell_y*BSIZE + cell_x)*2);
            
            -- 仅在有棋子时渲染
            if cell_state /= "00" then
                -- 计算中心坐标
            piece_x <= BOARD_OFFSET_X + cell_x*(CELL_SIZE + LINE_WIDTH)+ LINE_WIDTH + CELL_SIZE/2;
            piece_y <= BOARD_OFFSET_Y + cell_y*(CELL_SIZE + LINE_WIDTH)+ LINE_WIDTH + CELL_SIZE/2;
                
--                -- 距离计算
--                dist_sq := (x_pos - piece_x)**2 + (y_pos - piece_y)**2;
                
                -- 渲染逻辑
              if (abs(x_pos - piece_x) <= PIECE_RADIUS) and (abs(y_pos - piece_y) <= PIECE_RADIUS) then
                    piece_active <= '1';
...
--                    -- 最后一步高亮处理
                        ...
                        if (abs(x_pos - piece_x) > PIECE_RADIUS-10) and (abs(y_pos - piece_y) > PIECE_RADIUS-10) then
                        piece_red <= COLOR_WHITE;
